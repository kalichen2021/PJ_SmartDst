 ...
 * @param isVisibleVal 需要隐藏的元素组的显示状态（ref响应式变量）, mousedown事件赋值为false
 */
export const forStateClickSwhToHide = (
  willHiddenElement: itemOrArray<HTMLElement | string>,
  excludeElementGrp: itemOrArray<HTMLElement | string> = [],
  hideCallback?: Function,
) => {
...
export const UserOperaHandler = (AppGroupInstances: Map<string, any>) => {
  const _isExcludeEl = (targetEl: HTMLElement): Boolean => {
    let _r = false;
    AppGroupInstances.forEach((groupItem) => {
      groupItem.GrpCtnCtrlerList.forEach((ctrlerItem: Ref<HTMLElement>) => {
        if (isInDom(targetEl, ctrlerItem.value)) {
          _r = true;
          return;
        }
      })
  ...
  const _handler = (e: MouseEvent) => {
    ...
    if (_isExcludeEl(e.target as HTMLElement)) return;
    document.removeEventListener("mousemove", _handler)
    AppGroupInstances.forEach((groupItem) => {
      groupItem.state = "IDLE"
    })
    return;
  }
